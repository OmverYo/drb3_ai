...
class RobotController(Node):
    def __init__(self, mode: str = MODE):
        ...
        self.gripper2cam_path = os.path.join(package_path, "resource", "T_gripper2cam.npy")
        self.aruco_calculator = ArucoCalculator(
            node = self, get_current_posx_fn = get_current_posx, t_gripper_camera_path = gripper2cam_path
        )
        self.get_logger().info(f"ArucoCalculator initialized.")
    ...
    def _on_vision_response(self, future):
        """조건 2: 응답 성공/실패, 인식된 커맨드 값과 무관하게 항상 z축으로
        1cm 올라갔다가 원위치로 복귀한다.
        """
        try:
        ...
        try:
            ...
            self.get_logger().error(f"z축 bump 이동 실패: {e}")
        finally:
