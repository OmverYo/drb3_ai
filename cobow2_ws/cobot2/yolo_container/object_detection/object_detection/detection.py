...
class ObjectDetectionNode(Node):
    def __init__(self, model_name = 'yolo'):
        ...
        try:
            ...
            self.get_logger().error('SAM unavailable: ' + self.sam_error)
        self.grasp_planner = SamGraspPlanner(self.get_logger())
        self.create_service(SrvGraspPlan, 'get_grasp_plan', self.handle_get_grasp_plan)
    ...
    def _handle_grasp_plan(self, request, response):
        ...
        if self.sam is None:
            response.message = 'SAM unavailable: ' + self.sam_error
            return response
        ...
        plan = self.grasp_planner.plan(
            ...
            base_from_camera=request.base_from_camera if request.use_reference else None,
            reference_base_xy_mm=request.reference_base_xy_mm if request.use_reference else None,
        )
